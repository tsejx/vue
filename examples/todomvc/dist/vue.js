...
(function (global, factory) {
  ...
  function defineReactive$$1 (
    ...
    // 每个数据字段都通过了闭包引用着属于自己的 dep 常量
    // 因为在 walk 函数中通过循环遍历了所有数据对象的属性，并调用 defineReactive 函数，所以每次调用 defineReactive 定义访问器属性时，该属性的 setter /setter 都闭包引用了一个属于自己的容器
    ...
    Object.defineProperty(obj, key, {
      ...
      set: function reactiveSetter(newVal) {
        // 通过 getter 方法获取 prev 值，与新值进行比较，新旧值一致则不需要执行下面的操作（也就是监听值没变化）
        var value = getter ? getter.call(obj) : val;
...

  {
    ...
    // render 过程中有未在 data methods 或 props 中定义的属性
  ...
  function initRender (vm) {
    ...
    // Vue 框架内部 createElement 函数
    vm._c = function (a, b, c, d) { return createElement(vm, a, b, c, d, false); };
    // normalization is always applied for the public version, used in
    // user-written render functions.
    // 开发者定义的 createElement 函数
  ...
  var Watcher = function Watcher (
    vm, /* 组件实例对象 */
    expOrFn, /* 要观察的表达式 */
    cb, /* 当被观察的表达式的值发生变化时触发的回调函数 */
    options, /* 一些传递给当前观察者对象的选项 */
    isRenderWatcher /* 标识该观察者对实例是否是渲染函数的观察者（除了渲染函数的观察者，还有用户设定的观察则） */
  ) {
    // 组件实例
  ...
  // 判断当前标签是 input 时，type 是否相同
  // 某些浏览器不支持动态修改 input 类型，所以他们被是为不同类型
  ...
  function createPatchFunction (backend) {
    ...
    // vnode 根据 vnode 的数据结构创建真实的 DOM 节点，如果 vnode 有 children 则会遍历这些子节点，递归调用 createElm
    function createElm (
      vnode,
      insertedVnodeQueue,
      parentElm,
    ...
    function updateChildren (parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
      ...
      // removeOnly 运用于 transition-group 组件
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (isUndef(oldStartVnode)) {
          ...
          // 前四种情况是指定 key 时候，判定为同个 VNode，则直接 patchVnode 即可
          // 分别比较 oldChildren 以及 newChildren 的两头节点 2*2=4 种情况
          ...
          // 旧开始节点 与 新结束节点 相同，将该节点移动到新节点树最右
          // 则 旧开始节点 成为新末端节点，将 旧开始节点 插到 旧结束节点 后一个位置
          ...
          // 旧结束节点 与 新开始节点 相同
          // 则 旧结束节点 成为新的头端节点，将 旧结束节点 插到 旧开始节点前一个
          patchVnode(oldEndVnode, newStartVnode, insertedVnodeQueue, newCh, newStartIdx);
          canMove && nodeOps.insertBefore(parentElm, oldEndVnode.elm, oldStartVnode.elm);
          oldEndVnode = oldCh[--oldEndIdx];
          newStartVnode = newCh[++newStartIdx];
        } else {
          // 生成一个 key 与旧 VNode 的 key 对应的哈希表（只有第一次进来 undefined 的时候生成，也为后面检测重复的 key 值做铺垫）
          // 比如 children 是这样的 [{xx:xx,key:'key0'},{xx:xx,key:'key1'},{xx: xx, key: 'key2'}] beginIndex = 0 endIndex = 2
          ...
          if (isUndef(idxInOld)) { // New element
            // newStartVnode 没有 key 或者是该 key 没有在旧节点中找到，则创建新节点
            createElm(newStartVnode, insertedVnodeQueue, parentElm, oldStartVnode.elm, false, newCh, newStartIdx);
          } else {
            // 获取同 key 的旧节点
            vnodeToMove = oldCh[idxInOld];
            if (sameVnode(vnodeToMove, newStartVnode)) {
              // 如果新 VNode 与得到的有相同 key 的节点是同个 VNode 则进行 patchVnode
              patchVnode(vnodeToMove, newStartVnode, insertedVnodeQueue, newCh, newStartIdx);
              // 因为已经 patchVnode 进去了，所以这个旧节点赋值 undefined，之后如果还有新节点与该节点 key 相同可以检测出来提示已有重复的 key
              oldCh[idxInOld] = undefined;
              // 当有标识位 canMove 时可以直接插入 oldStartVnode 对应的真实 DOM 节点前面
              canMove && nodeOps.insertBefore(parentElm, vnodeToMove.elm, oldStartVnode.elm);
            } else {
              // same key but different element. treat as new element
              // 当新节点 VNode 与找到的同样 key 的 VNode 不是 sameVNode 时候（比如说 tag 不一样或者是有不一样 type 的 input 标签），创建一个新的节点
              createElm(newStartVnode, insertedVnodeQueue, parentElm, oldStartVnode.elm, false, newCh, newStartIdx);
            }
          }
          newStartVnode = newCh[++newStartIdx];
      ...
      if (oldStartIdx > oldEndIdx) {
        // 全部比较完成后，发现 oldStartIdx > oldEndIdx 的话，说明旧节点已经遍历完，新节点比旧节点多，所以这时多出来的新节点需要逐个（批量）创建出来加入到真实 DOM 中
        refElm = isUndef(newCh[newEndIdx + 1]) ? null : newCh[newEndIdx + 1].elm;
        addVnodes(parentElm, refElm, newCh, newStartIdx, newEndIdx, insertedVnodeQueue);
      } else if (newStartIdx > newEndIdx) {
        // 如果全部比较完成以后发现 newStartIdx > newEndIdx，则说明新节点已经遍历完成，新节点多于新节点，这个时候需要将多余的节点从真实 DOM 中移除
    ...
    // 比较新旧 vnode 节点，根据不同状态对 DOM 做合理的更新操作（添加、移动和删除）整个过程还会依次调用 prepatch、update、postpatch 等钩子函数，在编译阶段生成的一些静态子树，在这个过程中由于不会改变而直接跳过比对，动态子树在比较过程中比较核心的部分就是当新旧 vnode同时存在 children，通过 updateChildren 方法对子节点做更新
    function patchVnode (
      oldVnode,
      vnode,
      insertedVnodeQueue,
    ...
    // 打补丁函数（新旧虚拟节点不同时会执行该函数逻辑）
    // 流程包括 1. 创建新节点 2. 更新父占位符节点 3. 销毁旧节点
    // oldVnode 旧的虚拟节点
    // vnode 新的虚拟节点
    // hydrating
    // removeOnly
    return function patch(oldVnode, vnode, hydrating, removeOnly) {
      ...
      if (isUndef(oldVnode)) {
        ...
        // 根据就虚拟节点的 nodeType 判断是否为真实的 DOM
  ...
  function parse (
    ...
    function closeElement (element) {
      ...
      if (!stack.length && element !== root) {
        ...
        // 根级元素可以用 v-if、v-else 来写多个条件下的多个根级元素
        // 例如
        // <template>
        ...
        if (root.if && (element.elseif || element.else)) {
          ...
          // 在 el 的 ifConditions 属性中加入 condition
          addIfCondition(root, {
            exp: element.elseif,
            block: element
          });
        } else {
          // 在根级元素包含多个 ele 的时候，有不含 v-else 的 ele 则报出打印
          warnOnce(
      ...
      // forbidden 标识是否是被禁止的标签
      // style 标签或是 script 标签
      if (currentParent && !element.forbidden) {
        if (element.elseif || element.else) {
          // 当遇到当前 ele 有 v-else 或 v-else-if 属性时，需要处理 if 属性
          // 在其上个个兄弟元素中必然存在一个 v-if 属性
          ...
          if (element.slotScope) {
            ...
            // slot 如果没有则是默认的 defalt
            var name = element.slotTarget || '"default"'
              // scropedSlots 中存放 slot 元素
    ...
    parseHTML(template, {
      ...
      start: function start (tag, attrs, unary, start$1, end) {
        ...
        if (inVPre) {
          ...
          // 匹配 v-for 指令
          processFor(element);
          // 匹配 v-if 指令 分别处理 v-if v-else v-else-if
          processIf(element);
          // 处理 v-once
          processOnce(element);
        }
  ...
  // 处理原生属性，将其放入 attrs 中，以 {name, value} 的形式
  ...
  function parseFor (exp) {
    ...
    // 以 item in list 为例
    // isMatch = ['item of list', 'item', 'list', index: 0, input: 'item of list']
    var inMatch = exp.match(forAliasRE);
    // 匹配失败则在非生产环境中打印 v-for 的无效表达式
    if (!inMatch) { return }
    var res = {};
    // 这里是 list
    ...
    // 因为 item 可能被括号包裹 比如 (item, index) in list
    // 匹配 (item, index) 的结果
    // ['(item, index, l)', 'item', ' index', index, l, index: 0, input: '(item, index, l);' ]
  ...
  function processIf (el) {
    ...
    if (exp) {
      // 存在 v-if
      el.if = exp;
      // 在 el 的 ifConditions 属性中加入 {exp, block}
      addIfCondition(el, {
        exp: exp,
        block: el
      });
  ...
  function processIfConditions(el, parent) {
    // 当遇到当前 ele 有 v-else 或 v-else-if
    // 需要处理 v-if
    // 在其上个兄弟元素必然存在 v-if
  ...
  function processAttrs(el) {
    ...
    for (i = 0, l = list.length; i < l; i++) {
      ...
      if (dirRE.test(name)) {
        ...
        el.hasBindings = true;
        // modifiers
        // 解析表达式，比如 a.b.c.d 得到 {b: true, c: true, d: true}
        ...
        if (bindRE.test(name)) { // v-bind
          ...
          if (modifiers) {
            ...
            // .camel 将 kabab-case 特性名转换为 camelCase
        ...
        // 处理常规的字符串属性
        {
          ...
          if (res) {
            // 插入属性内部会被删除
            // 该用冒号或 v-bind
            // 比如应该用 <div :id="foo"> 代替 <div id="{{foo}}">
            warn$2(
              name + "=\"" + value + "\": " +
              'Interpolation inside attributes has been removed. ' +
              'Use v-bind or the colon shorthand instead. For example, ' +
  ...
  // 检测该元素是否在 for 循环中
  // 将会沿着 parent 元素逐级向上遍历寻找是否处于一个 for 循环中
